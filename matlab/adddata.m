...
for i = 1:length(ClusterData.(['H', name]).Cluster)
    if isempty(ClusterData.(['H', name]).Cluster(i).Cost)
        fprintf('Cluster %u has not been analyzed. \n', i)
        ...
        if ClusterData.(['H', name]).Cluster(i).mode_number > 1
            plot_original_data(ClusterData.(['H', name]).Cluster(i))
            plot_Clustering(ClusterData.(['H', name]).Cluster(i))
            ...
            prompt = input('Inspect heterogenity', 's');
            if isempty(prompt)
                ClusterData.(['H', name]).Cluster(i).proofreading = false;
            else
                ClusterData.(['H', name]).Cluster(i).proofreading = true;
            end
        else
            ClusterData.(['H', name]).Cluster(i).proofreading = false;
        end
