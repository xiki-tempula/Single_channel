...
for i = 1:length(start)
    ...
    %fix the wrong amplitude
    temp_amp = zeros(1,length(state));
    temp_amp(1:2:end) = amplitude(state == 1);
    temp_amp(2:2:end) = amplitude(state == 0);
    temp_amp = temp_amp(1:Clusterlength*2);
    amplitude = temp_amp;
